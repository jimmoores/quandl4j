 ...
 * <p>The core class is {@link QuandlSession}.  This can be initialized with or without {@link SessionOptions} that specify things 
 * like the Quandl API Token (optional, but strongly recommended - they are free).  The general pattern is that there is a Request 
 * and Reponse class.  The Request class often uses the Builder pattern to allow requests to be built in a fluent style.</p>
 * <pre>
 ...
 * <ul>
 *   <li>{@link DataSetRequest} - request tabular data set for a single Quandl code, builder has a range of optional parameters.</li>
 *   <li>{@link MultiDataSetRequest} - request tabular data across multiple Quandl codes, builder has a range of optional parameters.</li>
 *   <li>{@link MetaDataRequest} - request meta data for a single Quandl code.</li>
 *   <li>{@link MultiMetaDataRequest} - request meta data for multiple Quandl codes.</li>
 *   <li>{@link SearchRequest} - perform text-based searches, large result sets are paged.</li>
 * </ul>
 * <p>The core methods on the session are:</p>
 * <ul>
 *   <li>{@link QuandlSession#getDataSet(DataSetRequest) getDataSet(DataSetRequest)} - request tabular data for single Quandl code.</li>
 *   <li>{@link QuandlSession#getDataSets(MultiDataSetRequest) getDataSets(MultiDataSetRequest)} - request tabular data for multiple Quandl codes/columns.</li>
 *   <li>{@link QuandlSession#getMetaData(MetaDataRequest) getMetaData(MetaDataRequest)} - request meta data for a single Quandl code.</li>
 *   <li>{@link QuandlSession#getMetaData(MultiMetaDataRequest) getMetaData(MultiMetaDataRequest)} - request meta data for multiple Quandl codes.</li>
 *   <li>{@link QuandlSession#getMultipleHeaderDefinition(MultiMetaDataRequest) getMultipleHeaderDefinition(MultiMetaDataRequest)} - request column header definitions for multiple Quandl codes.</li>
 *   <li>{@link QuandlSession#search(SearchRequest) search(SearchRequest)} - free text search on available data.</li>
 * </ul>
 * <p>The data types returned by these methods are:</p>
 * <ul>
 *   <li>{@link TabularResult} - made up of a {@link HeaderDefinition} and a list of {@link Row}s.</li>
 *   <li>{@link MetaDataResult} - Wrapper around JSON object returned.  Contains various typed accessor methods for convenience.</li>
 *   <li>{@link SearchResult} - Wrapper around JSON object, allows easy iteration through resulting matches.</li>
 * </ul>
 ...
 *   {@link QuandlSession#getMultipleHeaderDefinition(MultiMetaDataRequest) getMultipleHeaderDefinition(MultiMetaDataRequest)}.</li>
 * </ul>
 */
